...
- [ ] determine/write minimum `layer` interfaces (*according to Anvil implementation*)
- [ ] consider the ways to test glsl renderers
- [ ] write unit tests
...
- Always use **primitive and flexible** way to implement texture handling package.
    ...
    #### what is flexible?
    - For example, when you're breaking down the stroke process into `start`, `add_point`, `end`, **avoid hard code the completion process (like `shape stamp` + `bresenham's line`) there**.
