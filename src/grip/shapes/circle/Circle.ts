...
import { CIRCLE_MERGE_300ES } from './shaders/merge';

/**
 * @description Circle shape that keeps constant opacity by merging a stroke mask over the base texture.
 */
export class CircleShape extends GripShape {
  readonly id = 'circle';
  ...
  private drawPoint(layer: Layer, x: number, y: number, style: GripStrokeStyle): void {
    ...
    const mask = this.ensureMask(layer);
    const bounds = makePointBounds(layer, x, y, radius);
    this.updateStrokeBounds(bounds);
    mask.applyEffect(
      {
