...
import type { LayerEventFor, LayerEventType } from '../layer/events';
...
export class LayerThumbnail {
  ...
  constructor(layer: Layer, options?: LayerThumbnailOptions) {
    ...
    this.onHistory = (event) => {
      ...
      this.updateTilesByBounds(event.bounds);
    };
  ...
  private updateAllTiles(): void {
    const tiles = this.layer.getTiles();
    this.updatePreviewTiles(tiles.map((tile) => tile.index));
  }

  private updateTilesByBounds(bounds: { x: number; y: number; width: number; height: number }): void {
    const tiles = this.layer.getTiles();
    const boundsX1 = bounds.x + bounds.width;
    const boundsY1 = bounds.y + bounds.height;
    const indices: number[] = [];
    for (const tile of tiles) {
      const tileX1 = tile.x + tile.width;
      const tileY1 = tile.y + tile.height;
      const x0 = Math.max(bounds.x, tile.x);
      const y0 = Math.max(bounds.y, tile.y);
      const x1 = Math.min(boundsX1, tileX1);
      const y1 = Math.min(boundsY1, tileY1);
      if (x1 <= x0 || y1 <= y0) continue;
      indices.push(tile.index);
    }
    this.updatePreviewTiles(indices);
  }

  private updatePreviewTiles(tileIndices: number[]): void {
    ...
    for (const index of tileIndices) {
      ...
      if (!tile) continue;
      const tex = this.layer.getTileTextureHandle(index);
      const dstX0 = Math.max(0, Math.floor(tile.x / this.scale));
      const dstY0 = Math.max(0, Math.floor(tile.y / this.scale));
      const dstX1 = Math.min(this.previewWidth, Math.ceil((tile.x + tile.width) / this.scale));
      const dstY1 = Math.min(this.previewHeight, Math.ceil((tile.y + tile.height) / this.scale));
      ...
      gl.blitFramebuffer(0, 0, tile.width, tile.height, dstX0, dstY0, dstX1, dstY1, gl.COLOR_BUFFER_BIT, gl.NEAREST);
    }
