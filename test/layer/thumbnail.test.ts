...
describe('LayerThumbnail', () => {
  ...
  it('downscales tiled layer with nearest sampling', () => {
    const gl = makeGL2Context(4, 4);
    const input = makeBlockPattern4x4();
    const layer = new Layer(gl, { width: 4, height: 4, data: input, tileSize: 2 });
  ...
  it('does not update when updateOn excludes history events', () => {
    ...
    layer.writePixels({ x: 0, y: 0, width: 2, height: 2 }, new Uint8Array(2 * 2 * 4));
  ...
  it('updates after historyRegistered when configured', () => {
    ...
    patch.fill(200);
    layer.writePixels({ x: 0, y: 0, width: 2, height: 2 }, patch);
